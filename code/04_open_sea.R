...
runoff_stats_class[, runoff_class := fifelse(mean_day >= 2000, "high",
                                             fifelse(mean_day >= 1000 & mean_day <= 2000, "medium", "low"))]
...
runoff_stations[, area_class := fifelse(area >= 130000, "large",
                                        fifelse(area >= 10000 & area <= 130000, "medium", "small"))]
...
runoff_stations[, alt_class := fifelse(altitude >= 400, "high",
                                       fifelse(altitude >= 50 & altitude <= 400, "medium", "low"))]
...
ggplot(runoff_winter[year > 1980], aes(x = year, y = value, col = sname)) +
  geom_line() +
  geom_point() + 
  scale_color_manual(values = colorRampPalette(colset_4)(n_stations)) +
  theme_bw()

ggplot(runoff_summer[year > 1980], aes(x = year, y = value, col = sname)) +
  geom_line() +
  geom_point() + 
  scale_color_manual(values = colorRampPalette(colset_4)(n_stations)) +
  theme_bw()
...
station_coords_sf <- st_as_sf(runoff_summary, 
                              coords = c("lon", "lat"), 
                              crs = 4326)
mapview(station_coords_sf, map.types = "CartoDB.Positron")
...
saveRDS(runoff_summary, './results/04/runoff_summary.rds')
saveRDS(runoff_stats, './results/04/runoff_stats.rds')
saveRDS(runoff_day, './results/04/runoff_day.rds')
saveRDS(runoff_month, './results/04/runoff_month.rds')
saveRDS(runoff_summer, './results/04/runoff_summer.rds')
saveRDS(runoff_winter, './results/04/runoff_winter.rds')
saveRDS(runoff_year, './results/04/runoff_year.rds')
